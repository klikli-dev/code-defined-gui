<!--
SPDX-FileCopyrightText: 2026 klikli-dev

SPDX-License-Identifier: MIT
-->

# Code-Defined GUI docs

## Guides

- [Architecture overview](./architecture-overview.md)
- [API surface and stability policy](./api-surface-and-stability-policy.md)
- [Using the library](./using-the-library.md)
- [Layout-driven quick start](./layout-driven-quick-start.md)
- [Premade filters](./premade-filters.md)
- [Examples guide](./examples-guide.md)
- [Styling premade GUIs](./styling-premade-guis.md)

## Recommended reading order

1. [Architecture overview](./architecture-overview.md) for the package map and stability boundary
2. [API surface and stability policy](./api-surface-and-stability-policy.md) for support expectations
3. [Layout-driven quick start](./layout-driven-quick-start.md) for the smallest end-to-end flow
4. [Using the library](./using-the-library.md) for the broader layout-driven workflow
5. [Premade filters](./premade-filters.md) for the shipped filter feature set
6. [Examples guide](./examples-guide.md) for reference code pointers
7. [Styling premade GUIs](./styling-premade-guis.md) for the generic `GuiStyle` system
