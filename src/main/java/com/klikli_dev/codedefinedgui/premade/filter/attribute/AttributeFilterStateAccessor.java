...
public final class AttributeFilterStateAccessor implements FilterStateAccessor<AttributeFilterState> {
    ...
    public AttributeFilterState read(ItemStack stack) {
        ItemContainerContents reference = stack.get(DataComponentsRegistry.ATTRIBUTE_FILTER_REFERENCE.get());
        AttributeFilterConfig config = stack.get(DataComponentsRegistry.ATTRIBUTE_FILTER_CONFIG.get());
    ...
    public void write(ItemStack stack, AttributeFilterState state) {
        if (state.referenceStack().isEmpty()) {
            stack.remove(DataComponentsRegistry.ATTRIBUTE_FILTER_REFERENCE.get());
        } else {
            stack.set(DataComponentsRegistry.ATTRIBUTE_FILTER_REFERENCE.get(), ItemContainerContents.fromItems(NonNullList.of(ItemStack.EMPTY, state.referenceStack().copyWithCount(1))));
        }
        ...
        if (AttributeFilterConfig.EMPTY.equals(config)) {
            stack.remove(DataComponentsRegistry.ATTRIBUTE_FILTER_CONFIG.get());
        } else {
            stack.set(DataComponentsRegistry.ATTRIBUTE_FILTER_CONFIG.get(), config);
        }
    }
}
